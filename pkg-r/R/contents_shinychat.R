...
#' inside dowshinychat by returning HTML objects directly from this method.
#'
#' You can also use this pattern to build upon the default dowshinychat display for
#' tool requests and results. By using [S7::super()], you can create the
#' object dowshinychat uses for tool results (or tool requests), and then modify it
#' to suit your needs.
#'
#' ```r
#' S7::method(contents_shinychat, WeatherToolResult) <- function(content) {
...
new_tool_card <- function(type, request_id, tool_name, ...) {
  ...
  classes <- c(
    paste0("dowshinychat_tool_", type),
...
#' @export
as.tags.dowshinychat_tool_card <- function(x, ...) {
  tag_name <- switch(
    x$type,
    request = "shiny-tool-request",
    result = "shiny-tool-result",
    cli::cli_abort(
      "dowshinychat tool card must have type {.val request} or {.val result}, not {.val {x$type}}."
    )
  )
...
#' @export
format.dowshinychat_tool_card <- function(x, ...) {
  format(as.tags(x), ...)
}

#' @export
print.dowshinychat_tool_card <- function(x, ...) {
  tags <- as.tags(x)
...
knit_print.dowshinychat_tool_card <- function(x, ...) {
