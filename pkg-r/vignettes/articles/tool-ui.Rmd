...
dowshinychat automatically displays rich UI for tool calls and results, providing users with an intuitive view of what tools are being executed and their outcomes. This tool UI works out-of-the-box with ellmer and requires no additional configuration to get started.
...
dowshinychat's tool UI works in the same way, but with rich output displays that are shown directly in the `chat_ui()` interface in Shiny apps.
When the LLM calls a tool, dowshinychat automatically displays the tool request in a collapsed card.
...
contents_dowshinychat(request)
```

When the tool result is completed, dowshinychat replaces the request with a card containing the tool result:
...
contents_dowshinychat(error_result)
```

When you use `chat_app()` or the chat UI module via `chat_mod_ui()` and `chat_mod_server()`, dowshinychat automatically handles tool requests and results, displaying them in the chat interface.

On the other hand, if you're using `chat_ui()` and calling `chat_append()` to stream the chat output, you'll need to make sure that ellmer streams tool requests and results to dowshinychat by setting `stream = "content"` in the `$stream_async()` call.
...
As a result, `ellmer::ContentToolRequest` and `ellmer::ContentToolResult` objects are streamed to dowshinychat, which automatically displays the tool requests and results in the chat interface.
...
If the tool annotations include a `title` or `icon`, dowshinychat will use them in the tool request and result displays.
...
contents_dowshinychat(request_with_annotations)
```
...
contents_dowshinychat(result_with_annotations)
```
...
dowshinychat looks for and uses a `display` list within `extra` to customize how the tool result is shown.
...
Now when the tool result is completed, dowshinychat shows the custom title and icon based on the forecasted temperature:
...
contents_dowshinychat(result_title_icon_result)
```
...
This strategy is so useful that dowshinychat automatically supports it for an `` `_intent` `` argument.
When `` `_intent` `` is present in the tool's arguments, dowshinychat shows this value in the tool request and result titles as the _reason the tool was called_.
...
When the tool is called, dowshinychat shows the reason the LLM called the tool in the header of the tool card.
...
contents_dowshinychat(result_random_number)
```
...
dowshinychat uses the `display` list for two additional categories of customization:
...
By default, dowshinychat shows the tool result's `value` property as a code block to the user.
...
dowshinychat looks for the following properties in the `display` list and shows the user the first available display in the following order:
...
2. `markdown`: A markdown string that is automatically rendered as rich HTML in dowshinychat.
...
contents_dowshinychat(result_html)
```
...
dowshinychat supports several options in the `display` list to control how results are presented:
...
dowshinychat uses the rich tool UI described above by default, but you can choose to hide all tool calls or use dowshinychat's basic display without allowing for rich or alternative content displays.
...
To adjust the display for debugging or verification purposes, set the `dowshinychat.tool_display` option (or the `SHINYCHAT_TOOL_DISPLAY` environment variable) to one of the following values:

```r
# Disable tool UI entirely
options(dowshinychat.tool_display = "none")

# Use basic text-based display (useful for verifying inputs/outputs)
options(dowshinychat.tool_display = "basic")

# Default rich display applies customizations from tool authors
options(dowshinychat.tool_display = "rich")
```
