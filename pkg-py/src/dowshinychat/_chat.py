...
class Chat:
    ...
    @overload
    def on_user_submit_files(self, fn: FileSubmitFunction) -> Effect_: ...

    @overload
    def on_user_submit_files(
        self,
    ) -> Callable[[FileSubmitFunction], Effect_]: ...

    def on_user_submit_files(
        self, fn: FileSubmitFunction | None = None
    ) -> Effect_ | Callable[[FileSubmitFunction], Effect_]:
        """
        Define a function to invoke when files are attached to a user submission.

        Apply this method as a decorator to a function that receives a list of
        file dicts when the user submits a message with attached files. Each file
        dict has ``name`` (str), ``type`` (MIME type str), and ``data``
        (base64-encoded str) fields.

        This only fires when ``file_input=True`` is set on the chat UI and the
        user attaches at least one file.

        Parameters
        ----------
        fn
            A function to invoke with the list of attached files.
        """

        def create_effect(fn: FileSubmitFunction):
            from shiny import reactive

            @reactive.effect
            @reactive.event(self._user_input_files)
            async def handle_files():
                try:
                    files = self._user_input_files()
                    if not files:
                        return
            ...
            self._effects.append(handle_files)
            return handle_files
    ...
    def _user_input_files(self) -> list[dict[str, str]] | None:
        id = f"{self.id}_user_input_files"
    ...
    async def update_slash_commands(
        ...
        """
        Dynamically update the available slash commands.

        Parameters
        ----------
        commands
            A list of slash command definitions. Each command is a dict with
            ``name`` (str), ``description`` (str), ``type`` (``"client"`` or
            ``"server"``), and optionally ``client_action`` (str).
        """
        action: ChatAction = {"type": "update_commands", "commands": commands}
...
class ChatExpress(Chat):
    def ui(
        ...
        **kwargs: TagAttrValue,
        ...
        file_input
            Enable file attachments. See :func:`chat_ui` for details.
        slash_commands
            A list of slash command definitions. See :func:`chat_ui` for details.
        message_actions
            Controls which action buttons appear on assistant messages. See
            :func:`chat_ui` for options.
        kwargs
...
def chat_ui(
    ...
    **kwargs: TagAttrValue,
    ...
    icon_assistant
            ...
            a default robot icon is used.
    file_input
        Enable file attachments via paste, drag-and-drop, and a file picker
        button. When ``True``, users can attach images, PDFs, and text files to
        their messages. Attached files are available as a separate reactive input
        at ``input.{id}_user_input_files``, which is a list of dicts, each with
        ``name`` (str), ``type`` (MIME type str), and ``data`` (base64 str)
        fields. This matches ellmer's ``ContentImageInline`` format. Defaults to
        ``False``.
    audio_input
    ...
    slash_commands
        A list of slash command definitions. Each command is a dict with ``name``
        (str), ``description`` (str), ``type`` (``"client"`` or ``"server"``),
        and optionally ``client_action`` (str). When the user types ``/`` in the
        input, a dropdown appears with matching commands.
    message_actions
        Controls which action buttons appear on assistant messages. Options:
    ...
    audio_input_attr: Optional[str] = None
    if audio_input is True or audio_input == "transcribe":
        audio_input_attr = "transcribe"
    elif audio_input == "raw":
        audio_input_attr = "raw"

    # Process file_input to an attribute value
    file_input_attr: Optional[str] = "true" if file_input else None

    # Process slash_commands to a JSON attribute
    import json

    slash_commands_attr: Optional[str] = None
    if slash_commands is not None:
        slash_commands_attr = json.dumps(slash_commands)
    ...
    res = Tag(
        ...
        Tag(
            ...
            id=f"{id}_user_input",
            placeholder=placeholder,
            audio_input=audio_input_attr,
            file_input=file_input_attr,
        ),
        ...
        slash_commands=slash_commands_attr,
        **kwargs,
    )
